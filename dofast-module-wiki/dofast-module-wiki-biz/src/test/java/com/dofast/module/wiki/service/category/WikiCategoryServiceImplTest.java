package com.dofast.module.wiki.service.category;

import org.junit.jupiter.api.Disabled;
import org.junit.jupiter.api.Test;
import org.springframework.boot.test.mock.mockito.MockBean;

import javax.annotation.Resource;

import com.dofast.framework.test.core.ut.BaseDbUnitTest;

import com.dofast.module.wiki.controller.admin.category.vo.*;
import com.dofast.module.wiki.dal.dataobject.category.WikiCategoryDO;
import com.dofast.module.wiki.dal.mysql.category.WikiCategoryMapper;
import com.dofast.framework.common.pojo.PageResult;

import javax.annotation.Resource;
import org.springframework.context.annotation.Import;
import java.util.*;
import java.time.LocalDateTime;

import static cn.hutool.core.util.RandomUtil.*;
import static com.dofast.module.wiki.enums.ErrorCodeConstants.*;
import static com.dofast.framework.test.core.util.AssertUtils.*;
import static com.dofast.framework.test.core.util.RandomUtils.*;
import static com.dofast.framework.common.util.date.LocalDateTimeUtils.*;
import static com.dofast.framework.common.util.object.ObjectUtils.*;
import static com.dofast.framework.common.util.date.DateUtils.*;
import static org.junit.jupiter.api.Assertions.*;
import static org.mockito.Mockito.*;

/**
 * {@link WikiCategoryServiceImpl} 的单元测试类
 *
 * @author 惠智造
 */
@Import(WikiCategoryServiceImpl.class)
public class WikiCategoryServiceImplTest extends BaseDbUnitTest {

    @Resource
    private WikiCategoryServiceImpl categoryService;

    @Resource
    private WikiCategoryMapper categoryMapper;

    @Test
    public void testCreateCategory_success() {
        // 准备参数
        WikiCategoryCreateReqVO reqVO = randomPojo(WikiCategoryCreateReqVO.class);

        // 调用
        Long categoryId = categoryService.createCategory(reqVO);
        // 断言
        assertNotNull(categoryId);
        // 校验记录的属性是否正确
        WikiCategoryDO category = categoryMapper.selectById(categoryId);
        assertPojoEquals(reqVO, category);
    }

    @Test
    public void testUpdateCategory_success() {
        // mock 数据
        WikiCategoryDO dbCategory = randomPojo(WikiCategoryDO.class);
        categoryMapper.insert(dbCategory);// @Sql: 先插入出一条存在的数据
        // 准备参数
        WikiCategoryUpdateReqVO reqVO = randomPojo(WikiCategoryUpdateReqVO.class, o -> {
            o.setId(dbCategory.getId()); // 设置更新的 ID
        });

        // 调用
        categoryService.updateCategory(reqVO);
        // 校验是否更新正确
        WikiCategoryDO category = categoryMapper.selectById(reqVO.getId()); // 获取最新的
        assertPojoEquals(reqVO, category);
    }

    @Test
    public void testUpdateCategory_notExists() {
        // 准备参数
        WikiCategoryUpdateReqVO reqVO = randomPojo(WikiCategoryUpdateReqVO.class);

        // 调用, 并断言异常
        assertServiceException(() -> categoryService.updateCategory(reqVO), CATEGORY_NOT_EXISTS);
    }

    @Test
    public void testDeleteCategory_success() {
        // mock 数据
        WikiCategoryDO dbCategory = randomPojo(WikiCategoryDO.class);
        categoryMapper.insert(dbCategory);// @Sql: 先插入出一条存在的数据
        // 准备参数
        Long id = dbCategory.getId();

        // 调用
        categoryService.deleteCategory(id);
       // 校验数据不存在了
       assertNull(categoryMapper.selectById(id));
    }

    @Test
    public void testDeleteCategory_notExists() {
        // 准备参数
        Long id = randomLongId();

        // 调用, 并断言异常
        assertServiceException(() -> categoryService.deleteCategory(id), CATEGORY_NOT_EXISTS);
    }

    @Test
    @Disabled  // TODO 请修改 null 为需要的值，然后删除 @Disabled 注解
    public void testGetCategoryPage() {
       // mock 数据
       WikiCategoryDO dbCategory = randomPojo(WikiCategoryDO.class, o -> { // 等会查询到
           o.setPid(null);
           o.setCategoryLevel(null);
           o.setName(null);
           o.setCreateTime(null);
       });
       categoryMapper.insert(dbCategory);
       // 测试 pid 不匹配
       categoryMapper.insert(cloneIgnoreId(dbCategory, o -> o.setPid(null)));
       // 测试 categoryLevel 不匹配
       categoryMapper.insert(cloneIgnoreId(dbCategory, o -> o.setCategoryLevel(null)));
       // 测试 name 不匹配
       categoryMapper.insert(cloneIgnoreId(dbCategory, o -> o.setName(null)));
       // 测试 createTime 不匹配
       categoryMapper.insert(cloneIgnoreId(dbCategory, o -> o.setCreateTime(null)));
       // 准备参数
       WikiCategoryPageReqVO reqVO = new WikiCategoryPageReqVO();
       reqVO.setPid(null);
       reqVO.setCategoryLevel(null);
       reqVO.setName(null);
       reqVO.setCreateTime(buildBetweenTime(2023, 2, 1, 2023, 2, 28));

       // 调用
       PageResult<WikiCategoryDO> pageResult = categoryService.getCategoryPage(reqVO);
       // 断言
       assertEquals(1, pageResult.getTotal());
       assertEquals(1, pageResult.getList().size());
       assertPojoEquals(dbCategory, pageResult.getList().get(0));
    }

    @Test
    @Disabled  // TODO 请修改 null 为需要的值，然后删除 @Disabled 注解
    public void testGetCategoryList() {
       // mock 数据
       WikiCategoryDO dbCategory = randomPojo(WikiCategoryDO.class, o -> { // 等会查询到
           o.setPid(null);
           o.setCategoryLevel(null);
           o.setName(null);
           o.setCreateTime(null);
       });
       categoryMapper.insert(dbCategory);
       // 测试 pid 不匹配
       categoryMapper.insert(cloneIgnoreId(dbCategory, o -> o.setPid(null)));
       // 测试 categoryLevel 不匹配
       categoryMapper.insert(cloneIgnoreId(dbCategory, o -> o.setCategoryLevel(null)));
       // 测试 name 不匹配
       categoryMapper.insert(cloneIgnoreId(dbCategory, o -> o.setName(null)));
       // 测试 createTime 不匹配
       categoryMapper.insert(cloneIgnoreId(dbCategory, o -> o.setCreateTime(null)));
       // 准备参数
       WikiCategoryExportReqVO reqVO = new WikiCategoryExportReqVO();
       reqVO.setPid(null);
       reqVO.setCategoryLevel(null);
       reqVO.setName(null);
       reqVO.setCreateTime(buildBetweenTime(2023, 2, 1, 2023, 2, 28));

       // 调用
       List<WikiCategoryDO> list = categoryService.getCategoryList(reqVO);
       // 断言
       assertEquals(1, list.size());
       assertPojoEquals(dbCategory, list.get(0));
    }

}
