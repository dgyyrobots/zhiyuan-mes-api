package com.dofast.framework.ip.core.utils;
...
public class AreaUtils {
    ...
    private static List<CsvRow> rows;
    ...
    private AreaUtils() {
        ...
        areas.put(Area.ID_GLOBAL, new Area(Area.ID_GLOBAL, "全球", 0,
                null, new ArrayList<>()));
        // 从 csv 中加载数据
        rows = CsvUtil.getReader().read(ResourceUtil.getUtf8Reader("area.csv")).getRows();
     ...
     * @return 区域ID
     */
    public static Integer search(String keyword, Integer level) {
        for (CsvRow row : rows) {
            if (level == Integer.valueOf(row.get(2)) && row.get(1).indexOf(keyword) >= 0) {
