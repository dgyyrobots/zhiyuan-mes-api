package com.dofast.module.system.service.social;
...
import com.dofast.module.system.dal.dataobject.social.SocialUserBindDO;
import com.dofast.module.system.dal.dataobject.social.SocialUserDO;
import com.dofast.module.system.dal.mysql.social.SocialUserBindMapper;
import com.dofast.module.system.dal.mysql.social.SocialUserMapper;
...
public class SocialUserServiceImpl implements SocialUserService {
    public static final ThreadLocal<DofastAuthRequestFactory> threadLocalFactory = new ThreadLocal<>();

    @Resource// 由于自定义了 DofastAuthRequestFactory 无法覆盖默认的 AuthRequestFactory，所以只能注入它
    private DofastAuthRequestFactory dofastAuthRequestFactory;
    ...
    public String getAuthorizeUrl(Integer type, String redirectUri) {
        threadLocalFactory.set(dofastAuthRequestFactory);
        // 在需要使用 DofastAuthRequestFactory 的地方，从线程局部变量中获取实例
        DofastAuthRequestFactory factory = threadLocalFactory.get();

        // 获得对应的 AuthRequest 实现
        AuthConfig authConfig = systemConfigService.getAuthConfig(SocialTypeEnum.valueOfType(type).getSource());
        if (authConfig == null){
            throw exception(CONFIG_NOT_EXISTS);
        }
        AuthRequest authRequest = factory.get(SocialTypeEnum.valueOfType(type).getSource());
        // 生成跳转地址
    ...
    private AuthUser getAuthUser(Integer type, String code, String state) {
        AuthRequest authRequest = dofastAuthRequestFactory.get(SocialTypeEnum.valueOfType(type).getSource());
