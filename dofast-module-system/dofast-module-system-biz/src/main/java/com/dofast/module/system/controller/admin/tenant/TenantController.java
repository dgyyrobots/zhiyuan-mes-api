package com.dofast.module.system.controller.admin.tenant;
...
import com.dofast.module.system.controller.admin.tenant.vo.tenant.*;
import com.dofast.module.system.convert.tenant.TenantConvert;
...
import com.dofast.module.system.dal.mysql.user.AdminUserMapper;
...
public class TenantController {
    ...
    @GetMapping("/get-tenant-by-domain-name")
    @PermitAll
    @Operation(summary = "使用域名，获得租户信息", description = "登录界面，根据域名，获得租户信息")
    @Parameter(name = "domainName", description = "域名", required = true, example = "1024")
    public CommonResult<TenantDO> getTenantByDomainName(@RequestParam("domainName") String domainName) {
        TenantDO tenantDO = tenantService.getTenantByDomainName(domainName);
        return success(tenantDO);
    }

    @GetMapping("/get-tenant-by-mobile")
    @PermitAll
    @Operation(summary = "根据手机号，获得租户", description = "登录界面，根据手机号，获得租户")
    @Parameter(name = "mobile", description = "手机号", required = true, example = "18437597802")
    public CommonResult<List<TenantDO>> getAllTenantMobile(@RequestParam("mobile") String mobile) {
        List<AdminUserDO> adminUserDOS = adminUserMapper.selectAllByMobile(mobile);
        ...
        for (AdminUserDO adminUserDO : adminUserDOS) {
            ...
            tenantDO.setLogo(tenant.getLogo());
            tenantDO.setSysName(tenant.getSysName());
            tenantDO.setName(tenant.getName());
            tenantDO.setDomain(tenant.getDomain());
            tenantDOS.add(tenantDO);
        }

        return success(tenantDOS);
    }
