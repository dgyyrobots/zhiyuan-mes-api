...
import com.dofast.module.trade.controller.admin.aftersale.vo.TradeAfterSaleDisagreeReqVO;
import com.dofast.module.trade.controller.admin.aftersale.vo.TradeAfterSalePageReqVO;
import com.dofast.module.trade.controller.admin.aftersale.vo.TradeAfterSaleRefuseReqVO;
import com.dofast.module.trade.controller.app.aftersale.vo.AppTradeAfterSaleCreateReqVO;
import com.dofast.module.trade.controller.app.aftersale.vo.AppTradeAfterSaleDeliveryReqVO;
...
import com.dofast.module.trade.dal.dataobject.aftersale.TradeAfterSaleLogDO;
...
import com.dofast.module.trade.dal.mysql.aftersale.TradeAfterSaleLogMapper;
import com.dofast.module.trade.dal.mysql.aftersale.TradeAfterSaleMapper;
import com.dofast.module.trade.enums.aftersale.TradeAfterSaleStatusEnum;
import com.dofast.module.trade.enums.aftersale.TradeAfterSaleTypeEnum;
import com.dofast.module.trade.enums.aftersale.TradeAfterSaleWayEnum;
...
public class TradeAfterSaleServiceImpl implements TradeAfterSaleService {
    ...
    @Autowired
    private TradeAfterSaleMapper AfterSaleMapper;
    @Autowired
    ...
    public PageResult<TradeAfterSaleDO> getAfterSalePage(TradeAfterSalePageReqVO pageReqVO) {
        return AfterSaleMapper.selectPage(pageReqVO);
    }
    ...
    private TradeAfterSaleDO validateAfterSaleAuditable(Long id) {
        TradeAfterSaleDO afterSale = AfterSaleMapper.selectById(id);
        if (afterSale == null) {
    ...
    private void updateAfterSaleStatus(Long id, Integer status, TradeAfterSaleDO updateObj) {
        int updateCount = AfterSaleMapper.updateByIdAndStatus(id, status, updateObj);
        if (updateCount == 0) {
    ...
    public void deliveryAfterSale(Long userId, AppTradeAfterSaleDeliveryReqVO deliveryReqVO) {
        // 校验售后单存在，并状态未退货
        TradeAfterSaleDO afterSale = AfterSaleMapper.selectById(deliveryReqVO.getId());
        if (afterSale == null) {
            throw exception(AFTER_SALE_NOT_FOUND);
        }
    ...
    public void refuseAfterSale(Long userId, TradeAfterSaleRefuseReqVO refuseReqVO) {
        // 校验售后单存在，并状态为已退货
        TradeAfterSaleDO afterSale = AfterSaleMapper.selectById(refuseReqVO.getId());
        if (afterSale == null) {
            throw exception(AFTER_SALE_NOT_FOUND);
        }
    ...
    public void refundAfterSale(Long userId, String userIp, Long id) {
        // 校验售后单的状态，并状态待退款
        TradeAfterSaleDO afterSale = AfterSaleMapper.selectByPayRefundId(id);
        if (afterSale == null) {
            throw exception(AFTER_SALE_NOT_FOUND);
        }
    ...
    private void createPayRefund(String userIp, TradeAfterSaleDO afterSale) {
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCommit() {
                ...
                // 更新售后单的退款单号
                AfterSaleMapper.updateById(new TradeAfterSaleDO().setId(afterSale.getId()).setPayRefundId(payRefundId));
            }
        });
    }
