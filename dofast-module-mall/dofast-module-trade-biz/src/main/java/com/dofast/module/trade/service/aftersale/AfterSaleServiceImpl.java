...
import com.dofast.module.pay.api.refund.PayRefundApi;
import com.dofast.module.pay.api.refund.dto.PayRefundCreateReqDTO;
import com.dofast.module.trade.controller.admin.aftersale.vo.AfterSaleDisagreeReqVO;
import com.dofast.module.trade.controller.admin.aftersale.vo.AfterSalePageReqVO;
import com.dofast.module.trade.controller.admin.aftersale.vo.AfterSaleRefuseReqVO;
import com.dofast.module.trade.controller.app.aftersale.vo.AppAfterSaleCreateReqVO;
import com.dofast.module.trade.controller.app.aftersale.vo.AppAfterSaleDeliveryReqVO;
...
import com.dofast.module.trade.dal.mysql.aftersale.AfterSaleMapper;
import com.dofast.module.trade.dal.redis.no.TradeNoRedisDAO;
import com.dofast.module.trade.enums.aftersale.AfterSaleOperateTypeEnum;
import com.dofast.module.trade.enums.aftersale.AfterSaleStatusEnum;
import com.dofast.module.trade.enums.aftersale.AfterSaleTypeEnum;
import com.dofast.module.trade.enums.aftersale.AfterSaleWayEnum;
...
import com.dofast.module.trade.framework.aftersale.core.annotations.AfterSaleLog;
import com.dofast.module.trade.framework.aftersale.core.utils.AfterSaleLogUtils;;
...
public class AfterSaleServiceImpl implements AfterSaleService {
    ...
    @Resource
    private AfterSaleMapper AfterSaleMappers;
    @Resource
    ...
    public PageResult<AfterSaleDO> getAfterSalePage(AfterSalePageReqVO pageReqVO) {
        return AfterSaleMappers.selectPage(pageReqVO);
    }

    @Override
    public PageResult<AfterSaleDO> getAfterSalePage(Long userId, PageParam pageParam) {
        return AfterSaleMappers.selectPage(userId, pageParam);
    }

    @Override
    public AfterSaleDO getAfterSale(Long userId, Long id) {
        return AfterSaleMappers.selectByIdAndUserId(id, userId);
    }

    @Override
    public AfterSaleDO getAfterSale(Long id) {
        return AfterSaleMappers.selectById(id);
    }
    ...
    private AfterSaleDO createAfterSale(AppAfterSaleCreateReqVO createReqVO,
        ...
        AfterSaleMappers.insert(afterSale);
    ...
    private AfterSaleDO validateAfterSaleAuditable(Long id) {
        AfterSaleDO afterSale = AfterSaleMappers.selectById(id);
        if (afterSale == null) {
    ...
    private void updateAfterSaleStatus(Long id, Integer status, AfterSaleDO updateObj) {
        int updateCount = AfterSaleMappers.updateByIdAndStatus(id, status, updateObj);
        if (updateCount == 0) {
    ...
    public void deliveryAfterSale(Long userId, AppAfterSaleDeliveryReqVO deliveryReqVO) {
        // 校验售后单存在，并状态未退货
        AfterSaleDO afterSale = AfterSaleMappers.selectById(deliveryReqVO.getId());
        if (afterSale == null) {
            throw exception(AFTER_SALE_NOT_FOUND);
        }
    ...
    public void refuseAfterSale(Long userId, AfterSaleRefuseReqVO refuseReqVO) {
        // 校验售后单存在，并状态为已退货
        AfterSaleDO afterSale = AfterSaleMappers.selectById(refuseReqVO.getId());
        if (afterSale == null) {
            throw exception(AFTER_SALE_NOT_FOUND);
        }
    ...
    private void createPayRefund(String userIp, AfterSaleDO afterSale) {
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCommit() {
                ...
                // 更新售后单的退款单号
                AfterSaleMappers.updateById(new AfterSaleDO().setId(afterSale.getId()).setPayRefundId(payRefundId));
            }
        });
    }
    ...
    public Long getApplyingAfterSaleCount(Long userId) {
        return AfterSaleMappers.selectCountByUserIdAndStatus(userId, AfterSaleStatusEnum.APPLYING_STATUSES);
    }
