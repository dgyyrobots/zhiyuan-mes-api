...
import com.dofast.module.promotion.convert.price.PriceConvert;
...
import com.dofast.module.promotion.enums.common.*;
...
import static com.dofast.framework.common.util.collection.CollectionUtils.getSumValue;
...
import static com.dofast.module.promotion.enums.ErrorCodeConstants.COUPON_NO_MATCH_MIN_PRICE;
import static java.util.Collections.singletonList;
import java.util.Collections;
import java.util.List;

import static com.dofast.framework.common.util.collection.CollectionUtils.getSumValue;
import static com.dofast.module.promotion.enums.ErrorCodeConstants.COUPON_VALID_TIME_NOT_NOW;
...
//                            .setMeetTip(String.format("差 %s 元可用优惠劵", formatPrice(coupon.getUsePrice() - originPrice)));
    ...
    private List<PriceCalculateRespDTO.OrderItem> getMatchCouponOrderItems(PriceCalculateRespDTO priceCalculate,
        ...
        return CollectionUtils.filterList(priceCalculate.getOrder().getItems(),
                orderItem -> coupon.getProductScopeValues().contains(orderItem.getSpuId()));
    }
     ...
     * 计算商品级别的价格，例如说：
     * 1. 会员折扣
     * 2. 限时折扣 {@link com.dofast.module.promotion.dal.dataobject.discount.DiscountActivityDO}
     *
     * 其中，会员折扣、限时折扣取最低价
     *
     * @param userId 用户编号
    ...
    private void calculatePriceByMemberDiscount(PriceCalculateRespDTO priceCalculate, PriceCalculateRespDTO.OrderItem orderItem,
                                                Integer memberPrice) {
        // 记录优惠明细
        addPromotion(priceCalculate, orderItem, null, PromotionTypeEnum.MEMBER_LEVEL.getName(),
                PromotionTypeEnum.MEMBER_LEVEL.getType(), PromotionLevelEnum.SKU.getLevel(), memberPrice,
    ...
    private void modifyOrderItemPayPrice(PriceCalculateRespDTO.OrderItem orderItem, Integer newPayPrice,
        ...
        order.setDiscountPrice(order.getDiscountPrice() - diffPayPrice);
    }
    ...
    private void addPromotion(PriceCalculateRespDTO priceCalculate, PriceCalculateRespDTO.OrderItem orderItem,
        ...
        PriceCalculateRespDTO.Promotion promotion = new PriceCalculateRespDTO.Promotion()
                ...
                .setItems(singletonList(promotionItem)).setMatch(meet).setDescription(meetTip);
    ...
    private void addPromotion(PriceCalculateRespDTO priceCalculate, List<PriceCalculateRespDTO.OrderItem> orderItems,
        ...
        PriceCalculateRespDTO.Promotion promotion = new PriceCalculateRespDTO.Promotion()
                ...
                .setItems(promotionItems).setMatch(meet).setDescription(meetTip);
        priceCalculate.getPromotions().add(promotion);
    ...
    /**
     * 计算订单级别的价格，例如说：
     * 1. 满减送 {@link com.dofast.module.promotion.dal.dataobject.reward.RewardActivityDO}
     *
     * @param userId 用户编号
     * @param priceCalculate 价格计算的结果
     */
    ...
    private void addNotMeetPromotion(PriceCalculateRespDTO priceCalculate, List<PriceCalculateRespDTO.OrderItem> orderItems,
        ...
        PriceCalculateRespDTO.Promotion promotion = new PriceCalculateRespDTO.Promotion()
                ...
                .setItems(promotionItems).setMatch(false).setDescription(meetTip);
        priceCalculate.getPromotions().add(promotion);
    }
